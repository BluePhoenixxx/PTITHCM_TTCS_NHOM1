...
{
    if (!MouseHooker::checkIsActivated(gControlData->_state.isClickActivated()) && !MouseHooker::checkIsActivated(gControlData->_state.isTrackingActivated()))
    ...
    if (nCode == 0)
    {
        ...
        if (wParam == WM_LBUTTONDOWN || wParam == WM_RBUTTONDOWN || wParam == WM_MOUSEWHEEL)
        {
